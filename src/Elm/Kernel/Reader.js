...
import Reader exposing (ModeBrowse, ModeDebug, update, view, parseConfig)
  ...
  return {
    init: function () {
      return A2(__Tuple_pair,
        A2(__Reader_parseConfig, __Reader_ModeBrowse, JSON.stringify(programData)),
        __Cmd_none);
    },
...
var _Reader_updateExec = F3(function (updateFn, msg, model)
{
  ...
  if (!_Reader_sourceMaps) {
    console.error("_Reader_sourceMaps uninitialized.");
  }
  var debugMetadata = {
    source_map: _Reader_sourceMaps,
    traces: _Reader_contextJSON(),
  };
  console.log("debugMetadata: ", debugMetadata);
  var readerModel = A2(__Reader_parseConfig, __Reader_ModeDebug, JSON.stringify(debugMetadata));
